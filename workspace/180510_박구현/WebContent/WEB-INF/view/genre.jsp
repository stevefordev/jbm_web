...
<body>
	...
	<div id="genreInputBox">
		...
		<form id="genreForm" action="/genre" method="post">
			...
			<div id="genreBox">
				<input id="name" name="name" placeholder="장르를 입력하세요"
					autocomplete="off" />
				<button id="registerBtn" title="입력">
		...
		<c:forEach items="${genres }" var="genre">
			<li class="genre">
				<h3 class="genre_title">
					<i class="no">${genre.no }</i> <span class="title">${genre.name }</span>
				</h3> <!--//.genre_title --> <span class="mania_nickname">${genre.maniaNickname }</span>
				<strong class="count_movie">${genre.movieCount}개</strong> <c:if
					test="${loginUser.no == genre.maniaNo }">
					<div class="delete">
						<form action="/genre" method="post">
							<input type="hidden" name="_method" value="DELETE"> <input
								type="hidden" name="no" value="${genre.no }">
							<button title="삭제">
	...
	<c:if test="${isHaveGenre }">
		<script>
			alert("이미 등록된 장르 이름 입니다.");
		</script>
	</c:if>
	<c:if test="${genreHasMovie }">
		<script>
			alert("장르와 연관된 영화가 있어서 삭제 불가능 합니다");
		</script>
	</c:if>
